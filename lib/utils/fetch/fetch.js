Fetch = function (_collection, _query, _options, wrapperFieldName = "data") {
  ...
  if (_options?.filtering) {
    const filterQuery = Object.keys(_options.filtering).reduce(function (
      obj,
      key
    ) {
      obj[key] = {
  ...
  if (_options?.pagination) {
    options.skip =
      (_options.pagination.currentPage - 1) * _options.pagination.pageItems;
  ...
  result.options.pagination.totalPages = Math.ceil(
    result.options.pagination.totalCount / result.options.pagination.pageItems
  );

  if (_options?.sorting) {
    options.sort = {};
    options.sort[_options.sorting.sortField] =
      _options.sorting.sortOrder === "asc" ? 1 : -1;
...
FetchByIndex = function (
  _collection,
  _query,
  _options,
  _index,
  wrapperFieldName = "data"
) {
  const result = {
    options: {
      pagination: {},
  ...
  if (_options?.pagination) {
    ...
    const totalPages = Math.ceil(
      result.options.pagination.totalCount / _options.pagination.pageItems
    );
    result.options.pagination.currentPage =
      _options.pagination.currentPage > totalPages
        ? 1
        : _options.pagination.currentPage;
    result.options.pagination.pageItems = _options.pagination.pageItems;
  }
